...
La intención es jugar un poco con los datos publicados en el [Portal de Datos Abiertos de la Nacion](http://datos.gob.ar). Especificamente el dataset "Solicitudes de Acceso a la Información Pública 2015" mientras pruebo la librería de visualización [rbokeh: R Interface to bokeh](http://hafen.github.io/rbokeh/).
...
plot <- figure(title = titulo, height = 640, width = 480) %>% 
  ly_bar(var, cantidad, data = df) %>% 
  theme_axis("x", major_label_orientation = 60)
plot
}
```
